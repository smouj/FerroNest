      ...
      <div className="text-center p-8 rounded-xl border border-red-900/50 bg-black/80 max-w-md">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6 text-center">
          ...
          <div>
            ...
            <div className="text-xs text-amber-200/60">Hatched</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-red-400">{totalLost}</div>
            <div className="text-xs text-amber-200/60">Lost</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-purple-400 capitalize">{gamePhase}</div>
            <div className="text-xs text-amber-200/60">Peak Phase</div>
          </div>
        </div>
        <button
          onClick={init}
...
function IntroScreen({ onStart }: { onStart: () => void }) {
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#050505] overflow-auto">
      <div className="text-center max-w-lg px-6 py-8">
        {/* Title */}
        <div className="mb-1">
          <span className="text-6xl font-bold tracking-tight bg-gradient-to-r from-amber-400 via-amber-600 to-amber-400 bg-clip-text text-transparent">
            FerroNest
          </span>
        </div>
        <p className="text-amber-200/30 text-sm mb-8 tracking-[0.3em] uppercase">
          Build the colony. Mark the trails. Protect the queen.
        </p>

        {/* Description */}
        <div className="bg-black/60 rounded-xl p-6 border border-amber-900/20 mb-6 text-left">
          <p className="text-amber-200/60 text-sm leading-relaxed mb-4">
            You don&apos;t control ants. You control a <span className="text-amber-400 font-bold">collective mind</span>.
          </p>
          <p className="text-amber-200/50 text-sm leading-relaxed mb-4">
            Dig tunnels, organize routes through <span className="text-purple-400 font-bold">pheromones</span>, raise new castes,
            cultivate fungus, and defend your queen against rain, predators, and rival colonies.
          </p>
          <div className="grid grid-cols-2 gap-2 text-xs text-amber-200/40 mt-4">
        ...
        {/* Controls */}
        <div className="bg-black/60 rounded-xl p-4 border border-amber-900/20 mb-8 text-left">
          <p className="text-amber-500 text-xs font-bold mb-2 uppercase tracking-wider">Controls</p>
          <div className="grid grid-cols-2 gap-1.5 text-xs text-amber-200/40">
        ...
        <button
          onClick={onStart}
          className="px-8 py-4 bg-gradient-to-r from-amber-700 to-amber-600 hover:from-amber-600 hover:to-amber-500 text-amber-100 font-bold text-lg rounded-lg transition-all transform hover:scale-105 shadow-lg shadow-amber-900/30"
        >
          Start Colony
        </button>
      </div>
...
export default function Home() {
  ...
  // Victory condition: survive 30 days
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-[#050505]">
      ...
      {hasWon && !showVictory && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="text-center p-8 rounded-xl border border-amber-500/50 bg-black/90 max-w-md">
            <h2 className="text-4xl font-bold text-amber-400 mb-2">Colony Thrives!</h2>
            <p className="text-amber-200/60 text-sm mb-4">Your colony has survived 30 days! The queen is strong and the colony grows.</p>
